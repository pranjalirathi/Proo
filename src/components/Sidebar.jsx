...
const Sidebar = ({ setIsSearchActive, setActiveComponent }) => {
  ...
  return (
    <div className={`fixed top-0 left-0 h-screen z-50 ${open ? 'w-56 bg-customBackground1 p-4' : isMobile ? 'fixed-width bg-customBackground1 p-2' : 'w-20 bg-customBackground1 pl-2 pr-1 pb-2'} pt-8 transition-width duration-300 flex flex-col justify-between`}>
      <div>
        <button
            ...
            >
              {/* <div className="w-6 h-6">{Menu.icon}</div>
                ...
                <div className="relative group w-6 h-6">
                  {Menu.icon}
                  <div id="tooltip-right" role="tooltip" className="absolute z-10 invisible group-hover:visible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 group-hover:opacity-100 tooltip dark:bg-gray-700"
                  >
                    {Menu.title}
