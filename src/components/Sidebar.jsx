...
const Sidebar = ({ setIsSearchActive, setActiveComponent }) => {
  ...
  return (
    <div className={`fixed top-0 left-0 h-screen z-50 ${open ? 'w-56 bg-customBackground1 p-4' : isMobile ? 'fixed-width bg-customBackground1 p-2' : 'w-20 bg-customBackground1 p-4'} pt-8 transition-width duration-300 flex flex-col justify-between`}>
      <div>
        <button
...
//       <div>
//         <button
//           className="absolute -right-4 top-11 sm:w-4 border-dark-purple border-2 rounded-full"
//           onClick={() => setOpen(!open)}
//         >
...
//           <img
//             src={coderoom1}
//             className={`h-10 w-10 sm:w-10 cursor-pointer duration-500 ${open && "rotate-[360deg]"}`}
//             alt="Coderoom Logo"
//             onClick={handleLogoClick}
//           />
//           <h1 className={`text-white origin-left font-bold text-xl duration-200 ${!open && "scale-0"}`}>
